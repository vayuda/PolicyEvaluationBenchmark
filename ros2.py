...
# create agents
target_policy = Agent(env)
target_policy.load_state_dict(model)
behavior_policy = Agent(env) 
...
with open(f'results/{wandb.config.env_id}/ros_return_estimate.csv', 'a', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(return_estimate)

with open(f'results/{wandb.config.env_id}/ros_steps.csv', 'a', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(steps)

with open(f'results/{wandb.config.env_id}/ros_mse.csv', 'a', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(mse)

with open(f'results/{wandb.config.env_id}/ros_norm_err.csv', 'a', newline='') as f:
    writer = csv.writer(f)
